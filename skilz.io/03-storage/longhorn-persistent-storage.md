...
Longhorn is block storage — one pod at a time. Some workloads (an app running several
replicas that share a common media or upload directory, for example) need multiple pods
to read and write the same volume simultaneously. That requires ReadWriteMany access
mode, which NFS provides.
...
### CSI plugin CrashLoopBackOff after a node restart
...
### Pod stuck in ContainerCreating, CSI socket missing
...
### Volume stuck Degraded, a replica on one node is unavailable
