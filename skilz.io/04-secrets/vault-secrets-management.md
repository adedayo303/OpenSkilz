...
- **Nothing is stored in YAML or Kubernetes Secrets** — applications request credentials
  at runtime, credentials are never written to disk
...
port 8500. No Consul was ever deployed — the error is Vault repeatedly failing to reach
a backend that does not exist.

**Fix:** Uninstall Vault, delete the PVCs (no data was ever written since Vault never
...
> Since Vault was never initialized (no data), a clean reinstall was safe.
...
> correct — the failure happens before the password is ever checked. `discoverdn=true` makes
...
This trips people up the first time they write a Vault policy, so it's worth calling out
...
That confirms the write landed and tells you this is version 1. Nothing else needs to
...
After this, the path behaves as if it was never written to. There's no CAS protection or
...
isn't needed for a small team, but it's there if a path ever needs that protection.
...
# 3. Confirm the new version landed
